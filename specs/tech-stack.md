...
| LLM | OpenAI `openai/gpt-5-6-luna` — TrueForge names the model with dashes, not the catalog's `gpt-5.6-luna` (confirmed at I1); the only supported model, no fallback | API key in TrueForge Settings → Models (never in repo) | — | B |
