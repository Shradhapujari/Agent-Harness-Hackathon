...
@mcp.tool()
@guarded
def list_pods(node: str | None = None, namespace: str = "demo") -> dict[str, Any]:
...
@mcp.tool()
@idempotent
def cordon_node(name: str, idempotency_key: str) -> dict[str, Any]:
...
@mcp.tool()
@idempotent
def uncordon_node(name: str, idempotency_key: str) -> dict[str, Any]:
...
@mcp.tool()
@idempotent
def drain_node(name: str, grace_s: int = 30, idempotency_key: str = "") -> dict[str, Any]:
    ...
    useful evidence, and the agent decides what to do about the remainder.
    """
    api().patch_node(name, {"spec": {"unschedulable": True}})
    evicted: list[str] = []
    blocked: list[str] = []
