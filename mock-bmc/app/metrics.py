...
def build_metrics_router(fleet: Fleet) -> APIRouter:
    ...
    def metrics() -> str:
        ...
        for name, (key, help_) in _GAUGES.items():
            ...
            out += [f'{name}{{system="{n["system_id"]}"}} {n[key]:.2f}' for n in nodes]
        out.append("# TYPE hush_power_on gauge")
        out += [
            f'hush_power_on{{system="{n["system_id"]}"}} {_bool(n["power"] == "On")}'
            for n in nodes
        ]
        out.append("# TYPE hush_host_hung gauge")
        out += [f'hush_host_hung{{system="{n["system_id"]}"}} {_bool(n["hung"])}' for n in nodes]
        out.append("# TYPE hush_thermal_trip gauge")
        out += [
            f'hush_thermal_trip{{system="{n["system_id"]}"}} {_bool(n["thermal_trip"])}'
            for n in nodes
        ]
        ...
        for n in nodes:
            for psu in (1, 2):
                out.append(
                    f'hush_psu_ok{{system="{n["system_id"]}",psu="{psu}"}} {_bool(n[f"psu{psu}_ok"])}'
                )
